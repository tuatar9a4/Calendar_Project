...
//        <a href="https://www.streamlinehq.com">Free Calendar Date PNG icon by Streamline</a> todayCllaendr
        ...
        for (a in weekText){
            if (a.equals("Sun")) weekFontStyle.setColor(ColorBuilders.argb(ContextCompat.getColor(baseContext,R.color.sunColor))).setSize(sp(10f))
            else if (a.equals("Sat"))weekFontStyle.setColor(ColorBuilders.argb(ContextCompat.getColor(baseContext,R.color.satColor))).setSize(sp(10f))
            else weekFontStyle.setColor(ColorBuilders.argb(ContextCompat.getColor(baseContext,R.color.currnetMonthDayColor))).setSize(sp(10f))
        ...
        calendarWeekTextSize = if (allDayOfMonth.size>35){ 12f  }else{ 14f  }
        ...
        for (i in 0 until totalDay) {
            ...
            if(i > 20){
                ...
                if (date.monthOfYear != DateTime(startValue.plusDays(i)).monthOfYear ){
                    ...
                    if(DateTime(startValue.plusDays(i)).dayOfWeek==6||DateTime(startValue.plusDays(i)).dayOfWeek==7) break;
                }
            }
        }
