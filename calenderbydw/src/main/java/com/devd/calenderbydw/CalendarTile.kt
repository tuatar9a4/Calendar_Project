...
class CalendarTile : TileService() {
    ...
    private var taskData = listOf<TaskDBEntity>()
    ...
    override fun onTileRequest(requestParams: RequestBuilders.TileRequest): ListenableFuture<TileBuilders.Tile> {
        if (sharedPreferences.getLong(settingMillis, 0) == 0L) {
            sharedPreferences.edit()
                .putLong(settingMillis, DateTime().withDayOfMonth(1).withTimeAtStartOfDay().millis)
                .apply()
        }
        ...
        Log.d("도원", "requestPareams dd : " + requestParams.state?.lastClickableId)
        if (requestParams.state?.lastClickableId != null) {
            if (requestParams.state!!.lastClickableId == applicationContext.getString(R.string.widget_minus)) {
                clickType = applicationContext.getString(R.string.widget_minus);
            } else if (requestParams.state!!.lastClickableId == applicationContext.getString(R.string.widget_plus)) {
                clickType = applicationContext.getString(R.string.widget_plus)
            } else if (requestParams.state!!.lastClickableId == applicationContext.getString(R.string.widget_today)) {
    ...
    override fun onCreate() {
        ...
        taskDb = TaskDatabase.buildDatabase(applicationContext)
        setCollectDBData()
    }

    private fun setCollectDBData() {
        CoroutineScope(Dispatchers.IO).launch {
            launch {
                taskDb.taskDao().getAllTask().collectLatest {
                    taskData= it
                }
            }
        }
    ...
    override fun onResourcesRequest(requestParams: RequestBuilders.ResourcesRequest): ListenableFuture<ResourceBuilders.Resources> {
        ...
        return Futures.immediateFuture(
            ResourceBuilders.Resources.Builder().setVersion(
                ...
                .addIdToImageMapping(
                    getString(R.string.dot_year), ResourceBuilders.ImageResource.Builder()
                ...
                .addIdToImageMapping(
                    getString(R.string.dot_month), ResourceBuilders.ImageResource.Builder()
                        .setAndroidResourceByResId(
                            ResourceBuilders.AndroidImageResourceByResId.Builder()
                ...
                .addIdToImageMapping(
                    getString(R.string.dot_day), ResourceBuilders.ImageResource.Builder()
                        .setAndroidResourceByResId(
                            ResourceBuilders.AndroidImageResourceByResId.Builder()
                ...
                .addIdToImageMapping(
                    getString(R.string.dot_week), ResourceBuilders.ImageResource.Builder()
                        .setAndroidResourceByResId(
                            ResourceBuilders.AndroidImageResourceByResId.Builder()
                ...
                .addIdToImageMapping(
                    getString(R.string.next_icon), ResourceBuilders.ImageResource.Builder()
                        .setAndroidResourceByResId(
                            ResourceBuilders.AndroidImageResourceByResId.Builder()
                ...
                .addIdToImageMapping(
                    getString(R.string.pre_icon), ResourceBuilders.ImageResource.Builder()
                        .setAndroidResourceByResId(
                            ResourceBuilders.AndroidImageResourceByResId.Builder()
    ...
    fun myLayout(deviceParameters: DeviceParametersBuilders.DeviceParameters): LayoutElementBuilders.LayoutElement {
        ...
        calendarLayout.setHeight(expand())
            ...
            .addContent(
                LayoutElementBuilders.Row.Builder().setWidth(wrap()).setVerticalAlignment(
                    ...
                    .addContent(
                        LayoutElementBuilders.Text.Builder().setMultilineAlignment(
                            LayoutElementBuilders.TextAlignmentProp.Builder().setValue(
                    ...
                    .addContent(
                        LayoutElementBuilders.Spacer.Builder().setWidth(dp(13f))
                            .setModifiers(
                                ModifiersBuilders.Modifiers.Builder()
                                    .setClickable(
                                        ModifiersBuilders.Clickable.Builder()
        ...
        for (a in weekText) {
            if (a.equals("Sun")) weekFontStyle.setColor(
                ColorBuilders.argb(
            ...
            else if (a.equals("Sat")) weekFontStyle.setColor(
                ColorBuilders.argb(
                    ContextCompat.getColor(
                        baseContext,
                        R.color.satColor
            ...
            else weekFontStyle.setColor(
                ColorBuilders.argb(
                    ContextCompat.getColor(
                        baseContext,
                        R.color.currnetMonthDayColor
                    )
                )
            ).setSize(sp(10f))
            ...
            if (!a.equals("Sat")) rowBuilderBox.addContent(
                LayoutElementBuilders.Spacer.Builder().setWidth(dp(calendarSpace)).build()
            )
        }
        //열에 아까 만든 요일 행 추가
        ...
        calendarWeekTextSize = if (allDayOfMonth.size > 35) {
            12f
        } else {
            14f
        }
        for (a in allDayOfMonth) {
            ...
            val dayTextColor =
                if (!strMonthDay.split(".")[0].equals(firstDayOfMonth.split(".")[0])) R.color.preMonthDayColor
...
//                }
//            }
            if (strs[2]=="Sun"  || strs[2]=="일")weekInt=1
            if (strs[2]=="Mon"  || strs[2]=="월")weekInt=2
            if (strs[2]=="Tue"  || strs[2]=="화")weekInt=3
            if (strs[2]=="Wed"  || strs[2]=="수")weekInt=4
            if (strs[2]=="Thu"  || strs[2]=="목")weekInt=5
            if (strs[2]=="Fri"  || strs[2]=="금")weekInt=6
            if (strs[2]=="Sat"  || strs[2]=="토")weekInt=7
            if (dotDaySetW.contains(strMonthDay)) {
                isDot = true
                Log.d("도원", "weekRepeat Dot : ${weekInt.toString()}")
                scheduleItem =
                    LayoutElementBuilders.Image.Builder().setHeight(dp(2f)).setWidth(dp(2f))
                        .setResourceId("dot_week")
                dotContainer.addContent(scheduleItem.build())
                    .addContent(LayoutElementBuilders.Spacer.Builder().setWidth(dp(1f)).build())
            }
            ...
            if (strMonthDay.equals(currentDate)) {
                calendarDayRow.addContent(
                    LayoutElementBuilders.Box.Builder().setWidth(dp(20f))
                        .setHeight(dp(calendarWeekTextSize + 8f))
                            ...
                            LayoutElementBuilders.Column.Builder()
                                .addContent(
                                    LayoutElementBuilders.Text.Builder()
                                        ...
                                        .setFontStyle(
                                            LayoutElementBuilders.FontStyles.caption1(
                                                deviceParameters
                                            )
                                                .setColor(
                                                    ColorBuilders.argb(
...
//                                    scheduleItem.build()
            ...
            count++;
            if (count == 7) {
                calendarLayout.addContent(
                    calendarDayRow.setModifiers(
                        ModifiersBuilders.Modifiers.Builder()
                            .setClickable(
                                ModifiersBuilders.Clickable.Builder()
                                    .setId("goToMain")
...
//        Log.d("도원","다음달 calendarDate : ${DateTime(plus).toString("yyyy.MM")} // test : ${DateTime(plus).toString("MM.dd")} //test2 : ${getMonthList(DateTime(plus))}  // ")
    ...
    //날짜를 얻어 봅시다
    private fun getDatDayFromDatabase(checkYear: String, checkMonth: String) {
        Log.d("Tile","getAllTask = >${taskData}")
        dotDaySetY.clear()
        dotDaySetM.clear()
        dotDaySetN.clear()
        dotDaySetW.clear()
        Log.d("Tile","weekCount = >  checkYear ${checkYear}   checkMonth :${checkMonth}  ")
        taskData.forEach { taskItem ->
            val month =
                if (taskItem.month.length == 1) "0${taskItem.month}" else taskItem.month
            val day = if (taskItem.day.length == 1) "0${taskItem.day}" else taskItem.day
            when (taskItem.repeatType) {
                0 -> { // 반복 없음
                    if (taskItem.year == checkYear && taskItem.month == checkMonth) {
                        Log.d("Tile","CheckDay0 = >${month}.${day}")
                        dotDaySetN.add("${month}.${day}")
                    }
                }

                1 -> { // 매일
                    dotDaySetN.add("${month}.${day}")
                    Log.d("Tile","CheckDay1 = >${month}.${day}")
                }

                2 -> { // 매주
                    Log.d("Tile","CheckDay2 = >${month}.${day}")
                    dotDaySetW.add("${taskItem.weekCount}")
                }

                3 -> { // 매달
                    dotDaySetM.add("${month}.${day}")
                }

                4 -> { // 매년
                    if (taskItem.month == checkMonth) {
                        Log.d("Tile","CheckDay4 = >${month}.${day}")
                        dotDaySetY.add("${month}.${day}")
                    }
                }
            }
        }
...
//        searchHolidayInDB(year.toInt(), month.toInt())
    ...
    private fun searchTaskOfRepeatYearInDB() {
        ...
        try {
            var c2: Cursor =
                database.rawQuery("SELECT month,day FROM myTaskTbl WHERE repeatY == 1", null);
            while (c2.moveToNext()) {
                val str1 =
                    if (c2.getInt(0).toString().length == 1) "0${c2.getInt(0)}" else c2.getInt(0)
                        .toString()
                val str2 =
                    if (c2.getInt(1).toString().length == 1) "0${c2.getInt(1)}" else c2.getInt(1)
                        .toString()
    ...
    fun searchTaskOfRepeatMonthInDB() {
        ...
        try {
            val c2: Cursor =
                database.rawQuery("SELECT day FROM myTaskTbl WHERE repeatM == 1", null);
            while (c2.moveToNext()) {
                val str1 =
                    if (c2.getInt(0).toString().length == 1) "0${c2.getInt(0)}" else c2.getInt(0)
                        .toString()
    ...
    private fun searchTaskOfRepeatNoInDB(currentYear: String) {
        ...
        try {
            val c2: Cursor =
                database.rawQuery(
                    "SELECT year,month,day FROM myTaskTbl WHERE repeatN == 1 AND year == ${currentYear}",
                    null
                );
            while (c2.moveToNext()) {
                val str1 =
                    if (c2.getInt(1).toString().length == 1) "0${c2.getInt(1)}" else c2.getInt(1)
                        .toString()
                val str2 =
                    if (c2.getInt(2).toString().length == 1) "0${c2.getInt(2)}" else c2.getInt(2)
                        .toString()
    ...
    private fun searchHolidayInDB(year: Int, month: Int) {
        ...
        try {
            if (TaskDatabaseHelper.isExistsTable(database, "holiday${year}Tbl")) {
                val c2: Cursor? =
                    TaskDatabaseHelper.searchHoliday(database, year, month, "holiday${year}Tbl")
                c2?.let {
                    while (it.moveToNext()) {
                        val holidayMonth = if (it.getInt(1)
                                .toString().length == 1
                        ) "0${it.getInt(1)}" else it.getInt(1)
                        val holidayDay = if (it.getInt(2)
                                .toString().length == 1
                        ) "0${it.getInt(2)}" else it.getInt(2)
    ...
    fun getMonthList(dateTime: DateTime): List<String> {
        ...
        Log.d("도원", "월 체크 :  getMonthList")
        for (i in 0 until totalDay) {
            list.add(DateTime(startValue.plusDays(i)).toString("MM.dd.E"))
            if (i > 20) {

                if (date.monthOfYear != DateTime(startValue.plusDays(i)).monthOfYear) {

                    if (DateTime(startValue.plusDays(i)).dayOfWeek == 6 || DateTime(
                            startValue.plusDays(
                                i
                            )
                        ).dayOfWeek == 7
